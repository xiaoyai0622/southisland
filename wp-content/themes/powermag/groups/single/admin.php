...
<div class="boxed">
	...
	<?php if ( bp_is_group_admin_screen( 'edit-details' ) ) : ?>
		...
		<p><input type="submit" value="<?php _e( 'Save Changes', 'buddypress' ); ?>" id="save" name="save" class="btn" /></p>
	...
	<?php if ( bp_is_group_admin_screen( 'group-avatar' ) ) : ?>
		...
		<?php if ( 'upload-image' == bp_get_avatar_admin_step() ) : ?>
				...
				<p>
					<input type="file" name="file" id="file" class="btn btn-inverse" />
					<input type="submit" name="upload" id="upload" class="btn" value="<?php _e( 'Upload Image', 'buddypress' ); ?>" />
		...
		<?php if ( 'crop-image' == bp_get_avatar_admin_step() ) : ?>
			...
			<h2><?php _e( 'Crop Avatar', 'buddypress' ); ?></h2>
	...
	<?php if ( bp_is_group_admin_screen( 'manage-members' ) ) : ?>
			...
			<ul id="admins-list" class="item-list single-line">
				...
				<li>
					...
					<h5>
						...
						</h5><span class="small pull-right">
							<a class="btn btn-mini confirm admin-demote-to-member" href="<?php bp_group_member_demote_link( bp_get_member_user_id() ); ?>"><?php _e( 'Demote to Member', 'buddypress' ); ?></a>
						</span>			
						<?php endif; ?>
		...
		<?php if ( bp_group_has_moderators() ) : ?>
			<div class="bp-widget">
				<h4 style="padding-top: 10px;"><?php _e( 'Moderators', 'buddypress' ); ?></h4>		
					...
					<ul id="mods-list" class="item-list single-line">
						...
						<li>
							...
							<span class="small pull-right">
								<a href="<?php bp_group_member_promote_admin_link( array( 'user_id' => bp_get_member_user_id() ) ); ?>" class="btn btn-mini confirm mod-promote-to-admin" title="<?php _e( 'Promote to Admin', 'buddypress' ); ?>"><?php _e( 'Promote to Admin', 'buddypress' ); ?></a>
								<a class="btn btn-mini confirm mod-demote-to-member" href="<?php bp_group_member_demote_link( bp_get_member_user_id() ); ?>"><?php _e( 'Demote to Member', 'buddypress' ); ?></a>
							</span>		
		...
		<div class="bp-widget">
			<h4 style="padding-top: 10px;"><?php _e("Members", "buddypress"); ?></h4>
			...
			<?php if ( bp_group_has_members( 'per_page=15&exclude_banned=false' ) ) : ?>
				...
				<ul id="group-members-list" class="item-list single-line">
					<?php while ( bp_group_members() ) : bp_group_the_member(); ?>
	
						<li class="clearfix <?php bp_group_member_css_class(); ?>">
							...
							<h5>
								...
								<?php if ( bp_get_group_member_is_banned() ) : ?>
									...
									<a href="<?php bp_group_member_unban_link(); ?>" class="btn btn-mini confirm member-unban" title="<?php _e( 'Unban this member', 'buddypress' ); ?>"><?php _e( 'Remove Ban', 'buddypress' ); ?></a>
	
								<?php else : ?>
	
									<a href="<?php bp_group_member_ban_link(); ?>" class="btn btn-mini btn-danger confirm member-ban" title="<?php _e( 'Kick and ban this member', 'buddypress' ); ?>"><?php _e( 'Kick &amp; Ban', 'buddypress' ); ?></a>
									<a href="<?php bp_group_member_promote_mod_link(); ?>" class="btn btn-mini confirm member-promote-to-mod" title="<?php _e( 'Promote to Mod', 'buddypress' ); ?>"><?php _e( 'Promote to Mod', 'buddypress' ); ?></a>
									<a href="<?php bp_group_member_promote_admin_link(); ?>" class="btn btn-mini confirm member-promote-to-admin" title="<?php _e( 'Promote to Admin', 'buddypress' ); ?>"><?php _e( 'Promote to Admin', 'buddypress' ); ?></a>
	
								<?php endif; ?>
	
									<a href="<?php bp_group_member_remove_link(); ?>" class="btn btn-mini confirm" title="<?php _e( 'Remove this member', 'buddypress' ); ?>"><?php _e( 'Remove from group', 'buddypress' ); ?></a>
	...
	<?php if ( bp_is_group_admin_screen( 'delete-group' ) ) : ?>
		...
		<div id="message" class="alert alert-danger">
			<p><?php _e( '<strong>WARNING:</strong> Deleting this group will completely remove ALL content associated with it. There is no way back, please be careful with this option.', 'buddypress' ); ?></p>
		</div>
		...
		<div class="submit">
			<input type="submit" disabled="disabled" class="btn btn-danger" value="<?php _e( 'Delete Group', 'buddypress' ); ?>" id="delete-group-button" name="delete-group-button" />
		</div>
