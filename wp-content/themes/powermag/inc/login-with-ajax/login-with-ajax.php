...
class LoginWithAjax {
	...
	public static function init(){
		...
		if( get_stylesheet_directory() != get_template_directory() ){
			self::find_templates( get_template_directory().'/inc/login-with-ajax/' );
		}
		...
		if ( !empty($_REQUEST["lwa"]) ) { //AJAX Request
			...
			if( !is_admin() ) {
				...
				/* djwd commented wp_enqueue_script( "login-with-ajax", self::locate_template_url($js_url), array( 'jquery' ) ); */
				
				/* djwd added */
				wp_enqueue_script( "login-with-ajax", get_template_directory_uri()."/inc/login-with-ajax/widget/login-with-ajax.js", array( 'jquery' ), '', true );

				/* djwd commented wp_enqueue_style( "login-with-ajax", self::locate_template_url('widget.css') );*/
			}
			...
			add_shortcode('powermag', 'LoginWithAjax::shortcode');
	...
	public static function login(){
		...
		if( !empty($_REQUEST['log']) && !empty($_REQUEST['pwd']) && trim($_REQUEST['log']) != '' && trim($_REQUEST['pwd'] != '') ){
			...
			if ( strtolower(get_class($loginResult)) == 'wp_user' ) {
				...
				$return['result'] = true;
				$return['message'] = __("Login Successful, redirecting...",'powermag');
				...
				if( !empty(self::$data['no_login_refresh']) && self::$data['no_login_refresh'] == 1 ){
					...
					$return['message'] = __("Login successful, updating...",'powermag');
				}
				...
				//Undefined Error
				$return['result'] = false;
				$return['error'] = __('An undefined error has ocurred', 'powermag');
			}
		}else{
			$return['result'] = false;
			$return['error'] = __('Please supply your username and password.', 'powermag');
		}
		$return['action'] = 'login';
	...
	public static function register(){
		...
		if ( !is_wp_error($errors) ) {
			//Success
			$return['result'] = true;
			$return['message'] = __('Registration complete. Please check your e-mail.','powermag');
	...
	public static function remember(){
		...
		if ( $result === true ) {
			...
			$return['result'] = true;
			$return['message'] = __("We have sent you an email", 'powermag');
	...
	public static function getLogoutRedirect(){
		$data = self::$data;
		//Global redirect
		$redirect = '%LASTURL%'; /* djwd changed "" to "%LASTURL%" for hard coded logout redirection */
	...
	public static function widget($instance = array() ){
		...
		if(is_user_logged_in()){
			...
			// /*djwd commented */ $template_loc = ($template_loc == '' && self::$template) ? self::$templates[self::$template].'/widget_in.php':$template_loc;
			...
			// /* djwd commented */ $template_loc = ($template_loc == '' && self::$template) ? self::$templates[self::$template].'/widget_out.php' : $template_loc;
...
function lwa_load_plugin_textdomain(){
	load_plugin_textdomain('powermag', false, "login-with-ajax/langs");
}
