...
function optionsframework_fields() {
	...
	foreach ( $options as $value ) {
		...
		case "info":
			...
			if ( isset($value['desc']) /* djwd added 'isset' */ ) {
		...
		case 'range':
			...
			<script>
			jQuery(function($) {
				$( "#slider-' . esc_attr( $value['id'] ) . '" ).slider({
					range: "min",
					value:'.esc_attr($val).',
					min:'.esc_attr( $value['min'] ).',
					max: '.esc_attr( $value['max'] ).',
					step: '.esc_attr( $value['step'] ).',
					slide: function( event, ui ) {
						$( "#amount-'. esc_attr( $value['id'] ) .'" ).val( ui.value );
					}
				});
				$( "#amount-'. esc_attr( $value['id'] ) .'" ).val( $( "#slider-' . esc_attr( $value['id'] ) . '" ).slider( "value" ) );
			});
			</script>
			
			';
			$output .= '<span class="range-wrap">
						
							
							<input id="amount-' . esc_attr( $value['id'] ) . '" class="of-input" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" type="text" value="' . esc_attr( $val ) . '"/>
							<span class="suffix">'. esc_attr( $value['suffix'] ) . '</span>
					
						</span>';
						
			$output .= '<div id="slider-'. esc_attr( $value['id'] ) .'"></div>';
